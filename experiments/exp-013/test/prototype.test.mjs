...
test('detached work progresses independently and exchanges sideband publications while pending', { timeout: 10_000 }, async () => {
  const box = mailbox();
...
test('application loop remains responsive while detached operation is active', { timeout: 10_000 }, async () => {
  const box = mailbox();
  box.hostStore(0, 1);
  const operation = new DetachedMockOperation({ mailbox: box, multiplierLane: 0, observationLane: 1, stopLane: 2 });
  let turns = 0;
...
test('controlled mock-device loss releases the mailbox lease but reports failure rather than cleanup success', { timeout: 10_000 }, async () => {
  const box = mailbox();
  box.hostStore(0, 1);
  const operation = new DetachedMockOperation({ mailbox: box, multiplierLane: 0, observationLane: 1, stopLane: 2 });
  await delay(15);
