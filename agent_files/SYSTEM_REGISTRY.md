...
| `project.public-repository` | Public-repository CI trust model, hardening assessment, security-setting state, and source-controlled public hygiene | [`../docs/PUBLIC_REPOSITORY.md`](../docs/PUBLIC_REPOSITORY.md) and [`../scripts/verify-public-repository.mjs`](../scripts/verify-public-repository.mjs) | Active informational/validation boundary |
...
| `docs.capability-status` | Non-shipped documentation/governance projection for independent capability dimensions, actual public export inventory, issue links, and CUDA-MCGS ownership/deletion-test state | [`../docs/capability-status.json`](../docs/capability-status.json) | Active #67 projection; exact public blocks/authority checked by `verify-docs`, never exported in `CUDA_JS_COMPATIBILITY` |
...
| `conformance.node` | Exact Node release registry, FFI/permission probes, profile-specific negative rows, and generated Node support list | [`../conformance/node/`](../conformance/node/README.md) and [`../docs/NODE_SUPPORT.md`](../docs/NODE_SUPPORT.md) | Active; exact Node 26.7.0 qualified, other listed releases are `testing-unconfirmed` or `known-incompatible` as recorded |
| `conformance.hardware` | Exact-profile support registry, architecture and independent extended-axis status dimensions, qualification orchestration, read-only Hyper-V readiness, evidence indexing/sanitization, and generated public hardware list | [`../conformance/hardware/`](../conformance/hardware/README.md) and [`../docs/HARDWARE_SUPPORT.md`](../docs/HARDWARE_SUPPORT.md) | Active; one accepted Windows x64 GPU profile, ordinary extended axes `not-qualified`, one exact Hyper-V profile `known-incompatible`, broader campaign open |
...
| `runtime.execution` | PTX/cubin module/function identity, packed launch, memory leases, private stream/event completion, and terminality | Accepted SPEC-0005/SPEC-0016 plus native conformance; concurrency needs accepted SPEC-0018 |
...
| `runtime.operation-scheduler` | Finite multiple in-flight operations, private streams, dependencies, hazards and backpressure | Accepted SPEC-0018 after trustworthy SPEC-0016 lifecycle evidence |
| `runtime.host-memory-transfer` | Pinned/registered host memory, bounded staging and asynchronous transfer operations | Accepted SPEC-0019 plus SPEC-0018 operation/dependency authority |
