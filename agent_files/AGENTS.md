...
platform truth and accepted authority
  ...
  -> LEGO component/public-contract ownership, studs/surfaces, and attention-bounded containment
...
**Recursive LEGO rule:** the application/system itself is the outermost LEGO. Its supported external inputs, outputs, commands, events, data contracts, and lifecycle entry/exit points are its public studs/surfaces. Large application sections, subsystems, components, and large objects should preferably be compositions of smaller child LEGOs when that preserves cohesion. Parents own their externally visible responsibility and hide child topology; callers connect only through deliberate studs/surfaces rather than drilling into private children.
