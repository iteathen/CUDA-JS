...
`CJS-F1A / EXP-000`, `CJS-F1B`, Windows-only `CJS-F2W / EXP-012`, `CJS-F3W`, `CJS-F4W`, `CJS-F5W`, `CJS-F6W / EXP-009`, and `CJS-F7W` are accepted on exact host, ABI, Driver, GPU, permission, oracle, actor-affinity, resource, memory, execution, compiler/linker/cache, platform-diagnostic, stress, and cleanup evidence. The F3 through F7 control plane also passes in native Linux CI without establishing Linux Driver support. F8 packaging/public-facade work requires an accepted detailed contract; Linux `CJS-F2L / EXP-001` through F7L remain present, deferred, and incomplete.
