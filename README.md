...
Windows-only `CJS-F8W` has exact accepted evidence. The public `cuda-js` 0.1.0-alpha.9 testing package exposes one OS-neutral safe runtime facade, immutable compatibility metadata, `discoverCudaDevices()`, explicit opaque device selection, allocation-owned contiguous typed views, a standalone `compileDeviceProgram()` helper, and a mock-only testing entry. Native Linux x86-64 is source-admitted only as `testing-unconfirmed`; the complete EXP-001/F1B/F3L-F8L native runner and evidence-validation chain is ready, but Linux remains not-qualified until the exact Ubuntu package and native-oracle chain passes. Clean tarball install/uninstall, first-consumer deletion, unrelated consumers, simultaneous runtime isolation, cross-runtime rejection, installed-package Windows vector execution, device-scope `u32`/`u64` multiword publication, and SPEC-0014 mailbox publication pass. The package also carries portable/software implementations of SPEC-0010 typed RDC, SPEC-0011 `u64`/`i32`/`f32` scalar arguments, SPEC-0012 Device LTO, SPEC-0013 restricted Device-JS plus its accepted SPEC-0022 and SPEC-0014 children, SPEC-0016 opaque operations, SPEC-0017 selected-device/target orchestration, and SPEC-0021 typed-view creation/use/cleanup; exact-profile qualification remains capability-specific. [`conformance/f8/README.md`](conformance/f8/README.md) gives the native Linux qualification handoff.
