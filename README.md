...
- **Real CUDA stream/event execution today.** The qualified legacy terminal-launch path uses a private `CU_STREAM_NON_BLOCKING` stream and private CUDA events. The implemented SPEC-0016 public surface adds one opaque `submit()` → `status()` / `wait()` / `close()` operation lifecycle while retaining one pending operation; exact Windows evidence covers delayed completion, deferred failure attribution, and teardown on the recorded profile.
- **Current one-pending-operation profile is not the architectural ceiling.** One public runtime currently admits one pending GPU operation for deterministic ordering/error attribution. Multiple in-flight operations and private multi-stream scheduling are planned but neither implemented nor qualified; streams, events, and operations remain CUDA-JS-owned resource families.
...
Currently not-implemented or not-qualified families include multiple in-flight operations/private multi-stream scheduling, public stream/event objects, multi-GPU/MIG, managed/pinned/mapped/pool memory, CUDA Graph execution, graphics interop, external contexts, process isolation, broader kernel signatures, and native Linux CUDA execution. Their absence from the current qualified profile does **not** mean the target architecture rejects them; each advances its architecture, implementation, qualification, and priority dimensions independently.
