...
Windows-only `CJS-F8W` retains exact accepted evidence for earlier recorded package profiles. The current public `cuda-js` 0.1.0-alpha.13 testing candidate keeps the OS-neutral facade and adds SPEC-0030 dense Device-JS scalars after the portable/package SPEC-0020 semantic prepared-DAG capability, SPEC-0028 library composition, and optional SPEC-0023/SPEC-0029 cuBLASLt f32 row-major matmul profile. Its additive portable surface continues SPEC-0011 scalar arguments, SPEC-0012 typed Device LTO, SPEC-0013 restricted Device-JS through `compileDeviceProgram()` / `compileDeviceLibrary()`, and SPEC-0017 opaque selection through `discoverCudaDevices()`. Clean tarball install/uninstall, first-consumer deletion, two unrelated dense-numeric consumers, simultaneous runtime isolation, cross-runtime rejection, opaque selected devices/views, Device-JS composition, semantic prepared-DAG replay, and mock library lifecycle pass portable controls. Exact Windows installed-package CUDA 13.3/compute_75/GTX 1660 Ti dense `f64`/`f16`/`bf16` output matches a separately compiled CUDA C++ oracle and closes terminally; cuBLASLt retains its separate exact profile. Native Linux x86-64 remains `testing-unconfirmed`, and other providers, CUDA Graph realization, tensor operations/cores, fast math, and performance remain unqualified or unimplemented. [`conformance/f8/README.md`](conformance/f8/README.md) gives the native Linux qualification handoff.
