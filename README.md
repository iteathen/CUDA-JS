...
- **A one-pending-operation default with an exact bounded widening.** The compatibility default remains one pending GPU operation. Opting into SPEC-0018 admits exactly two pending operations on two private streams with declared hazards and one predecessor; SPEC-0019 composes bounded snapshot H2D, terminal-result D2H, and D2D operations through the same lifecycle. Neither profile exposes streams, events, pointers, or an unbounded queue.
...
- **Typed compiler and device-language extensions are implemented without native overclaim.** The public/package path includes typed relocatable PTX, `u64`/`i32`/`f32` scalar arguments, typed `lto-ir` compilation and homogeneous Device-LTO linking, restricted Device-JS with explicit relaxed device-scope `u32`/`u64` atomic observation, and the opaque SPEC-0016 operation lifecycle. Exact native claims apply only to the recorded capability/profile evidence; other additive families retain their own gates.
...
Currently not-implemented or not-qualified families include public stream/event objects, caller-owned registered or mapped host memory, multi-GPU/MIG, managed/pool memory, CUDA Graph execution, graphics interop, external contexts, process isolation, broader kernel signatures, and native Linux CUDA execution. Their absence from the current qualified profile does **not** mean the target architecture rejects them; each advances its architecture, implementation, qualification, and priority dimensions independently.
...
Windows-only `CJS-F5W` is accepted. The runtime copies bounded PTX, resolves exact declared function schemas, packs naturally aligned arguments, retains all resource leases, and resolves only after private events report terminal completion. The default is one private stream/operation; exact SPEC-0018 qualification adds a capacity-two/two-stream/no-queue profile, and SPEC-0019 adds two internal pinned staging blocks with contiguous H2D/D2H/D2D operations. Independent MSVC and Node paths agree on scalar, delayed-event, atomic-observer, and async-transfer oracles with terminal cleanup. [`conformance/f5/README.md`](conformance/f5/README.md) preserves the human Linux adapter/oracle/evidence work without claiming native Linux CUDA support.
...
Windows-only `CJS-F8W` has exact accepted evidence. The public `cuda-js` 0.1.0-alpha.6 testing package exposes a safe runtime facade, immutable compatibility metadata, a standalone `compileDeviceProgram()` helper, and a mock-only testing entry. Clean tarball install/uninstall, first-consumer deletion, unrelated consumers, simultaneous runtime isolation, cross-runtime rejection, and installed-package Windows vector execution pass. The package also carries portable/software implementations of SPEC-0010 typed RDC, SPEC-0011 `u64`/`i32`/`f32` scalar arguments, SPEC-0012 Device LTO, SPEC-0013 restricted Device-JS plus the accepted SPEC-0022 scoped-atomic-observation child, and SPEC-0016 opaque operations; exact-profile qualification remains capability-specific. [`conformance/f8/README.md`](conformance/f8/README.md) gives the native Linux adapter and qualification handoff.
