...
Windows-only `CJS-F8W` has exact accepted evidence. The public `cuda-js` 0.1.0-alpha.5 testing package exposes a safe runtime facade, immutable compatibility metadata, a standalone `compileDeviceProgram()` helper, and a mock-only testing entry. Clean tarball install/uninstall, first-consumer deletion, unrelated consumers, simultaneous runtime isolation, cross-runtime rejection, and installed-package Windows vector execution pass. The package also carries portable/software implementations of SPEC-0010 typed RDC, SPEC-0011 `u64`/`i32`/`f32` scalar arguments, SPEC-0012 Device LTO, SPEC-0013 restricted Device-JS, and SPEC-0016 opaque operations; those additive capabilities retain separate native promotion gates. [`conformance/f8/README.md`](conformance/f8/README.md) gives the native Linux adapter and qualification handoff.
