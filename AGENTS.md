...
The repository is in an **active implementation phase** with `CJS-F1A`, `CJS-F1B`, Windows-only `CJS-F2W / EXP-012`, `CJS-F3W`, `CJS-F4W`, `CJS-F5W`, and Windows `CJS-F6W / EXP-009` accepted. The platform-neutral F3 through F6 controls also pass without native Linux CUDA providers, but native Linux Driver/compiler execution remains blocked on `CJS-F2L / EXP-001` and qualified external hardware. Windows F7 work requires a new accepted detailed specification. EXP-000 remains the mandatory exact Node 26.7.0 Windows x64/native Linux x86-64 regression capsule.
