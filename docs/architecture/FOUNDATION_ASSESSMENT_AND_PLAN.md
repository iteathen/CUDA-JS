...
The project owner authorized dependency-ordered implementation and a Windows-first sequence. `CJS-F1A / EXP-000` is promoted after independent Windows x64 and native Linux x86-64 qualification. `CJS-F1B` is accepted after deterministic CUDA 13.3.29 import, reviewed Tier-0 semantics, generated products, native layouts, and mutation-sensitive regeneration. Windows `CJS-F2W / EXP-012`, `CJS-F3W`, and synchronous copied-memory `CJS-F4W` are accepted on exact Driver/GPU/C-oracle/permission/actor/resource/byte/cleanup evidence. The F3/F4 control plane passes in native Linux CI, but Linux `CJS-F2L / EXP-001` and native Linux DriverActor/memory execution remain retained, deferred, and incomplete. Windows F5 contract work is dependency-ready; later work remains gated on its platform-specific predecessors.
