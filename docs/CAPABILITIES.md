...
- portable/software/package implementations of typed relocatable PTX, SPEC-0011 `u64`/`i32`/`f32` scalar launch arguments, SPEC-0021 `f64`/`f16`/`bf16` scalar launch arguments, typed Device LTO, restricted Device-JS with SPEC-0030 dense scalar computation and typed leaf-library composition, scoped atomic observation, device-scope release/acquire publication and direction-specific mailbox publication, and one opaque pending-operation lifecycle, with exact native status tracked per capability/profile;
...
- multi-GPU, MIG, managed memory, caller-registered/mapped host memory, CUDA Graph realization, graphics interop, external contexts, process isolation, arbitrary kernel signatures beyond the accepted closed parameter kinds, and native Linux CUDA execution are not currently qualified public capabilities; the SPEC-0020 semantic prepared-DAG fallback and bounded SPEC-0031 cuBLASLt child are implemented, with qualification remaining exact-profile-specific;
...
The accepted compiler surface includes three path-free trusted-header profiles. `cuda-cccl` verifies and snapshots the exact manifest-owned CUDA 13.3 `cuda/` and `nv/` roots. SPEC-0030 adds `cuda-numeric`, the finite top-level dependency closure for `cuda_fp16.h` and `cuda_bf16.h`, plus `cuda-device`, the exact ordered composite of numeric headers and CCCL. Components are inventoried before cache lookup, composite identity includes every ordered component digest, duplicate logical names and caller shadowing fail closed, and no ambient include path is accepted.
...
SPEC-0028 adds typed Device-JS leaf libraries. `compileDeviceLibrary()` accepts device-function-only source plus explicit exports and returns a copied RDC or LTO library record. A composed `compileDeviceProgram()` request imports exact exports through explicit local aliases, snapshots and validates every artifact before program compilation, and reuses CompilerActor linking for the final cubin. Symbols are deterministic consequences of library identity rather than caller-selected native names. Nested libraries, ambient registries, CUDA source, tensors, and native-control escape hatches remain excluded; native composition is not yet qualified.

SPEC-0030 adds an exact separate dense numeric contract selected only when metadata or helpers use `f64`, `f16`, or `bf16`. These kinds work as pointers, locals, same-unit device-function and leaf-library signatures, and kernel scalars through the existing SPEC-0021 ABI. Exact casts define integer modulo/two's-complement behavior, round-to-nearest-even floating conversions, and saturating truncation from floating values. Half/bfloat arithmetic uses named CUDA round-to-nearest intrinsics after every operation; typed NaN/infinity, `abs`, `isNaN`, elementary math, numeric `min/max`, and NaN-propagating `minimum/maximum` are finite helpers. Unchanged legacy requests retain byte-identical lowering and identity. Tensor shapes/operations, tensor-core selection, fast math, and performance policy remain outside Device-JS.
...
| Device-JS dense numeric profile | `planned` | `implemented` | `qualified` | `active` | SPEC-0030 exact `f64`/`f16`/`bf16` scalar computation and trusted numeric/device headers; two unrelated installed portable consumers plus independently authored CUDA C++ oracle parity on the recorded Windows CUDA 13.3/compute_75/GTX 1660 Ti profile. Linux/other profiles and tensor/performance claims remain unqualified. |
| Typed Device-JS library composition | `planned` | `implemented` | `not-qualified` | `active` | SPEC-0028 leaf libraries, explicit aliased imports, deterministic symbols, homogeneous RDC/LTO artifacts and final cubin; portable/package evidence only. |
...
| Explicit device selection | `planned` | `implemented` | `not-qualified` | `active` | SPEC-0017 portable/software/package path: finite sanitized `discoverCudaDevices()`, opaque process-local selectors, one selected device per runtime, and selected-target identity; native default/explicit and distinct-device evidence remain open. |
...
| Prepared operation DAGs / CUDA Graphs | `planned` | `implemented` | `qualified` | `active` | SPEC-0020 provides immutable 1–32-node kernel DAGs; SPEC-0031 adds fixed SPEC-0029 cuBLASLt f32 nodes with derived accesses. Both use named replay bindings, canonical identity, one whole-DAG operation, and one single-stream semantic realization. Qualification is limited to the exact recorded Windows mixed installed-package cell; CUDA Graph realization remains not implemented and no speedup is claimed. |
