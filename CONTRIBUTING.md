...
`CJS-F1A / EXP-000` remains a required regression capsule. `CJS-F1B` is accepted with pinned CUDA header import, generated ABI facts, reviewed Tier-0 semantics, normalized Runtime IR products, and independent native C ABI probes. Windows-only `CJS-F2W / EXP-012` and `CJS-F3W` are accepted. The F3 platform-neutral capsule passes on native Linux, but Linux `CJS-F2L / EXP-001` and native Linux DriverActor execution remain incomplete and contribution-ready; follow [`experiments/exp-001/README.md`](experiments/exp-001/README.md), coordinate through [issue #4](https://github.com/iteathen/CUDA-JS/issues/4), and do not claim Linux Driver support without its qualified native evidence.
