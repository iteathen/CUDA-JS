...
`CJS-F1A / EXP-000` remains a required regression capsule. `CJS-F1B` is accepted with pinned CUDA facts and independent native probes. Windows-only `CJS-F2W / EXP-012` through `CJS-F6W / EXP-009` are accepted on their bounded Driver, memory, execution, compiler, linker, cache, and cleanup evidence. Portable F3 through F6 controls pass without native Linux CUDA providers, but Linux `CJS-F2L / EXP-001` through F6L remain incomplete and contribution-ready; follow the retained conformance runbooks, coordinate through [issue #4](https://github.com/iteathen/CUDA-JS/issues/4), and do not claim Linux support without qualified native evidence.
