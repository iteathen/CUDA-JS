...
export const nativeRoot = path.join(repositoryRoot, 'build', 'f5', profileName, 'native');
export const nativeProfile = process.platform === 'win32' ? 'windows' : process.platform === 'linux' ? 'linux' : 'unsupported';
export const oraclePath = path.join(nativeRoot, `launch-oracle${process.platform === 'win32' ? '.exe' : ''}`);
...
export const capabilityOraclePath = path.join(nativeRoot, `capability-oracle${process.platform === 'win32' ? '.exe' : ''}`);
export const nativeCapabilitiesEvidenceName = `native-${nativeProfile}-capabilities.json`;
