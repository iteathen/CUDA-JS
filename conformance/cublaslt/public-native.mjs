...
try {
  ...
  console.log(JSON.stringify({ provider: adapter.provider, workspaceBytes: plan.workspaceBytes, status: terminal.status, output: [...new Float32Array(output.bytes.buffer, output.bytes.byteOffset, 4)] }));
} finally {
  ...
  if (!terminal.graceful) throw new Error('Public cuBLASLt native fixture did not close gracefully.');
}
