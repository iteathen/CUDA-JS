...
export const evidenceRoot = path.join(repositoryRoot, 'build', 'f9', profileName, 'evidence');
export const sourcePath = path.join(repositoryRoot, 'conformance', 'f9', 'fixtures', 'atomic-publication.cu.txt');

export function digestBytes(bytes) { return createHash('sha256').update(bytes).digest('hex'); }
export async function sha256(filePath) { return digestBytes(await readFile(filePath)); }
