...
Only exact Node 26.7.0 is currently `qualified-experimental`. Other Node 26 releases that pass the probe remain `no-support` until the complete synthetic and native qualification chain passes on that exact release. Node versions without the required FFI substrate are verified negative profiles.
