...
if (host.hostKind === 'linux-native-x64') assert.equal(host.action, 'run-linux-native-f2l-through-f8l');

await writeEvidence('linux-readiness.json', {
  schemaVersion: 1,
  workPackage: 'CJS-F7L',
  capsule: 'native-linux-platform-handoff-readiness',
  status: host.hostKind === 'linux-native-x64' ? 'diagnostic-source-ready-not-qualified' : 'prepared-not-qualified',
  ...
  remainingGates: host.hostKind === 'linux-native-arm64'
    ...
    : ['run retained F2L through F6L native Driver/GPU/provider gates', 'complete and run F7 permission and repeated native lifecycle evidence'],
  claimLimits: ['Host classification and diagnostics source readiness only.', 'No Driver, compiler, GPU, WSL, or native Linux CUDA success claim.'],
});
