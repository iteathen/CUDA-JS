...
try {
  ...
  const prepared = await runtime.prepareOperationDag([{
    ...
    block: { x: 128, y: 1, z: 1 },
    arguments: [{ binding: 'output' }, { binding: 'left' }, { binding: 'right' }, count],
    accesses: [
      { argumentIndex: 0, byteOffset: 0, byteLength, mode: 'write' },
      { argumentIndex: 1, byteOffset: 0, byteLength, mode: 'read' },
      { argumentIndex: 2, byteOffset: 0, byteLength, mode: 'read' },
    ],
  }]);
  preparedIdentity = { contract: prepared.contract, nodeCount: prepared.nodeCount, edgeCount: prepared.edgeCount, realization: prepared.realization };
  const operation = await prepared.submit({ bindings: { output, left: leftMemory, right: rightMemory } });
  const completion = await operation.wait();
  assert.equal(completion.status, 'completed');
  assert.equal(completion.kind, 'prepared-batch');
...
assert.deepEqual(preparedIdentity, {
  contract: 'SPEC-0020-prepared-kernel-dag-v1',
  nodeCount: 1,
  edgeCount: 0,
...
console.log(JSON.stringify({ consumer: 'native-prepared-dag', checksum: 15_600_773, preparedIdentity, graceful: terminal.graceful }));
