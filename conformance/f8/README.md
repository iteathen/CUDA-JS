...
The F8 capsule builds the exact `cuda-js` tarball with Node 26.7.0, inspects its contents, installs it into clean unrelated consumer directories, runs only exported entry points, tests two simultaneous runtimes and cross-runtime rejection, uninstalls the package, and verifies package-owned files are removed. Portable installed consumers exercise the current additive public surface: SPEC-0010 relocatable PTX, SPEC-0011 `u64`/`i32`/`f32` launch arguments, SPEC-0012 typed LTO-IR/Device-LTO linking, SPEC-0013 standalone Device-JS compilation, and SPEC-0016 opaque submit/status/wait/close operations. Those portable checks prove package/facade orchestration only; each capability retains its own native promotion gate.
