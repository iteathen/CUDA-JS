# Package conformance runbook

The F8 capsule builds the exact `cuda-js` tarball with Node 26.7.0, inspects its contents, installs it into clean unrelated consumer directories, runs only exported entry points, tests simultaneous runtimes and cross-runtime rejection, uninstalls the package, and verifies package-owned files are removed. Portable installed consumers exercise the current additive public surface, including SPEC-0010 relocatable PTX, SPEC-0011 scalar arguments, SPEC-0012 Device LTO, SPEC-0022 device publication, SPEC-0014 publication mailboxes, SPEC-0016/0018 operations, SPEC-0019 bounded asynchronous transfers, SPEC-0021 typed views, SPEC-0028 Device-JS libraries, SPEC-0030 dense numeric Device-JS, SPEC-0020 semantic prepared kernel-DAG replay, and the optional SPEC-0023/SPEC-0029 cuBLASLt f32 matmul lifecycle through the mock provider. Two unrelated installed consumers compile dense requests through public exports. Portable checks prove package/facade orchestration only; prepared replay does not prove CUDA Graph realization or performance, mock compilation/library calls do not prove a native provider, and every capability retains its own native promotion gate.

On the accepted Windows x64 profile, the installed package runs the tracked PTX vector kernel, the exact capacity-two atomic producer/observer, snapshot H2D→D2D→terminal-result D2H, and a source-only Device-JS mailbox kernel through the public facade. Vector, transfer, and mailbox publication retain independent F5 C-oracle coverage. The same tarball also runs a consumer-neutral Device-JS fixture covering structured control flow, exact scalar/atomic behavior, device-scope `u32`/`u64` release/acquire publication of an immutable multiword payload, private compiler/Driver composition, and aggregate cleanup. SPEC-0030 adds a separate installed-package fixture with mixed `f64`/`f16`/`bf16` pointers, locals, same-unit device calls, kernel scalars, casts, arithmetic, signed zero, NaN propagation, and math. Its exact bytes match an independently authored and separately compiled CUDA C++ oracle. A separate CUDA-free protocol oracle covers immutable-message and unrelated work-slot consumers plus early-ready, pre-acquire, partial-payload and stale/wrong-generation negatives. Every consumer reports graceful Worker exit and zero unproved live resources.
...
4. Run `npm run f8:native` and `npm run f8:verify`. The shared runner installs the same tarball and runs the vector, Device-JS, dense numeric, multi-operation, transfer, and mailbox consumers against their independent oracles; package self-comparison cannot replace an oracle.
