...
if (!existsSync(node)) {
  console.error(`CJS-F3 requires official Node ${requestedVersion} at ${node} or CUDA_JS_NODE.`);
  process.exit(2);
}
...
if (version.status !== 0 || version.stdout.trim() !== requestedVersion) {
  console.error(`CJS-F3 requires ${requestedVersion}; selected executable reports ${version.stdout.trim() || 'no version'}.`);
  process.exit(2);
}
