...
if (!existsSync(node)) {
  console.error(`CJS-F5 requires official Node ${requestedVersion} at ${node} or CUDA_JS_NODE.`);
  process.exit(2);
}
...
if (version.status !== 0 || version.stdout.trim() !== requestedVersion) {
  console.error(`CJS-F5 requires ${requestedVersion}; selected executable reports ${version.stdout.trim() || 'no version'}.`);
  process.exit(2);
}
...
if (!(action in steps)) {
  console.error(`Unknown CJS-F5 action: ${action}`);
  process.exit(2);
}
...
for (const step of steps[action]) {
  ...
  const result = spawnSync(node, step.args, { cwd: repositoryRoot, env: { ...process.env, CUDA_JS_F5_NODE: node }, stdio: 'inherit' });
