...
Native Windows usage requires exact Node 26.7.0 with its experimental FFI flag. `compile()` and `link()` serialize in the CompilerActor but do not block the application event loop. Call `close()` and verify `graceful: true` before considering native cleanup proved.
